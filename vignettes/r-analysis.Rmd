...
```
```{r determine_filter_threshold, echo=FALSE, eval=TRUE}
example_se_filtering_assessment
```
...
se_dge_shrink
```
```{r log2_shrinkage, echo=FALSE, eval=TRUE}
head(se_dge_shrink)
```
...
```
```{r gene_regulation_summary, echo=FALSE, eval=TRUE}
DESeq2_gene_reg_summary
```
...
example_se_volcano
```
```{r generate_volcano, echo=FALSE, eval=TRUE}
example_se_volcano 
```
